...
def tel_read_counts(open_bam, row):
    ''' Function for counting reads in telomeres. Called by generate_metatel_df by way of compile_telomere_series'''
    s = pd.Series(1, index=np.arange(60000))
    if row['name'].endswith('L'):
        reads = open_bam.fetch(row['chromosome'],0,60000)
    elif row['name'].endswith('R'):
        reads = open_bam.fetch(row['chromosome'],row['end']-60000,row['end'])
...
def compile_telomere_series((bam, cen_tel_gff3)):
    '''Function for compiling telemere series. Called by generate_metatel_df'''
    print bam
    ...
    for ix, r in cen_tel.iterrows():
        if r['name'].lower().startswith('tel'):
            tel_s_dict[r['name']] = tel_read_counts(open_bam, r)
    
    tel_s_dict['total aligned reads'] = GT.count_aligned_reads(bam)
    
    return (bam, tel_s_dict)

def create_meta_telomere(chip_dict, wce_dict, chip_name):
    '''Function for creating metatelomere from telomere series (generated by compile_telomere_series).
    Called by generate_metatel_df.'''
    avg_tel = pd.Series(0, index=np.arange(60000))
    for tel, s in chip_dict.items():
        if tel != 'total aligned reads':
            s = s.divide(chip_dict['total aligned reads']) / wce_dict[tel].divide(wce_dict['total aligned reads'])
            avg_tel = avg_tel+s
    avg_tel = avg_tel.divide(len(chip_dict)-1)
    avg_tel.name = chip_name
    return avg_tel

def generate_metatel_df(chip_wce_pairs, out_name, threads=4, tel_len=60000, cen_tel_gff3='/home/jordan/GENOMES/Cen_Tel.gff3.txt'):
    ...
    for chip, wce in chip_wce_pairs:
        bam_set.add((wce, cen_tel_gff3))
        bam_set.add((chip, cen_tel_gff3))
    bam_list = list(bam_set)
    if len(bam_list) < threads: threads = len(bam_list)
    p = Pool(threads)
    tel_series = p.map(compile_telomere_series, bam_list)
    
    chip_ser = {x[0]:x[1] for x in tel_series if x[0] in zip(*chip_wce_pairs)[0]}
    wce_ser = {x[0]:x[1] for x in tel_series if x[0] in zip(*chip_wce_pairs)[1]}
    ...
    for chip, chip_dict in chip_ser.items():
        ...
        s = create_meta_telomere(chip_dict, wce_dict, chip_name)
        df.loc[:,s.name] = s
        print df
    df.to_csv(out_name+'.csv')
    return df

def plot_metatelomere(df, samples, out_name, colors=['#222f3e','#ee5253','#5f27cd','#10ac84'], smooth_window=1000):
    ...
    ax.set_xlabel('Distance from telomere end (bp)')
